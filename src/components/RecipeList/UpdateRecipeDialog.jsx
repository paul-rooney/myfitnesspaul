...
const UpdateRecipeDialog = ({ recipe, ingredients }) => {
    ...
    return (
        <Dialog id="updateRecipeDialog" title="Update recipe" submitHandler={submitHandler}>
            <Stack>
                {recipe?.recipes_sources?.source && <h3 style={{ fontSize: "var(--font-size-0)", fontWeight: "var(--font-weight-4)" }}>{recipe.recipes_sources.source}</h3>}
                ...
                */}
                <Input id="page_number" label="Page number" defaultValue={pageNumber} changeHandler={changeHandler} />
                <Input id="display_name" label="Display name" required defaultValue={recipe.display_name} />
                ...
                <Stack space="var(--size-2)">
                    {recipe &&
                        ...
                        recipe.recipes_ingredients.map((item, index) => (
                            <Fragment key={index}>
                                <Cluster align="baseline" space="var(--size-2)">
                                    <span style={{ fontSize: "var(--font-size-0)", marginInlineEnd: "auto" }}>{item.ingredients.display_name}</span>
                ...
                <fieldset style={{ borderBlockEnd: "0", borderInline: "0", paddingInline: "0" }}>
                    <legend style={{ fontSize: "var(--font-size-0)" }}>Add ingredient</legend>
                    <Stack>
                        ...
                        <Input placeholder="Leave blank to use the typical unit weight" id="unit" label="Unit (optional)" list="units" autoCapitalize="off" />
                        <datalist id="units">
                            <option>g</option>
                            <option>ml</option>
                            <option>tbsp</option>
